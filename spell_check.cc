...
void SpellCheck(HashTable<string> & hash_table, const string & document_filename, const string & dictionary_filename)
{
   	...
   	while(documentFile >> original_word)
	{
		...
		word = LowerCase(word); // Turns all letters into lowercase
		if(!hash_table.Contains(word)) // if the word is spelled incorrectly (can't be found in hash table)
		{
			...
			if(case_a == true)
				cout << original_word << " -> " << new_word << " (case a) " << endl;
			else if(case_b == true)
				cout << original_word << " -> " << new_word << " (case b) " << endl;
			else if(case_c == true)
				cout << original_word << " -> " << new_word << " (case c) " << endl;
			else
				cout << original_word << " -> CANNOT BE CORRECTED." <<endl;	
...
int main(int argc, char **argv) {
  ...
  HashTable<string> hashing_table;
  SpellCheck(hashing_table, document_filename, dictionary_filename);
